...
def get_video_details(youtube, video_id, upload_id):
    ...
    def time_duration(t):
        ...
        if 'H' in time_string:
            hours_index = time_string.index('H')
            hours = int(time_string[:hours_index])
            time_string = time_string[hours_index + 1:]

        if 'M' in time_string:
            minutes_index = time_string.index('M')
            minutes = int(time_string[:minutes_index])
            time_string = time_string[minutes_index + 1:]

        if 'S' in time_string:
            seconds_index = time_string.index('S')
            seconds = int(time_string[:seconds_index])
...
def display_sample_data(channel_id):
    ...
    sample_output = {'channel_name': channel_data1, 'playlists': playlist_data1, 'videos': []}

    for i, video_id in enumerate(total_video_ids1):
        video_data1 = get_video_details(youtube, video_id, upload_id1)
        video_with_comments = {'video_data': video_data1, 'comments': []}

        if int(video_data1['comment_count']) > 0:
            comments_data1 = get_comments_details(youtube, video_id)
            video_with_comments['comments'] = comments_data1

        sample_output['videos'].append(video_with_comments)
        if i >= 4:  # Display data for at most 5 videos
            break
...
def list_mongodb_collection_names(database):
    she = myclient = pymongo.MongoClient("mongodb+srv://sherly:sherly123@cluster0.zhil3bq.mongodb.net/")
    db = she[database]
...
def sql_channel(database, col_input):
    she_mdb = she = myclient = pymongo.MongoClient("mongodb+srv://sherly:sherly123@cluster0.zhil3bq.mongodb.net/")
    db = she_mdb[database]
    col = db[col_input]
...
def channels_totalchannels():
    ...
    she_s = pymysql.connect(host='localhost', port=3307,user='root', password='153@sherly',        database='y_tb')
    cursor = she_s.cursor()
...
def q9_channelnames_avgvideoduration():
    she_sql = pymysql.connect(host='localhost', port=3307,user='root', password='153@sherly',        database='y_tb')
    cursor = she_sql.cursor()
    cursor.execute("SELECT\
                    channel.channel_name,\
                    TIME_FORMAT(SEC_TO_TIME(AVG(TIME_TO_SEC(video.duration))), '%H:%i:%s') \
                    AS average_duration\
                FROM\
                    video\
                INNER JOIN playlist ON playlist.upload_id = video.upload_id\
